...
def _compute_estimated_wait(parties) -> int:
...
def _party_to_response(party, parties) -> PartyResponse:
    position = party.position or 0
    waiting_parties = [p for p in parties if p.status == PartyStatus.waiting and (p.position or 0) < position]
...
async def create_party(body: CreatePartyRequest):
    ...
    await store.add_action_log(party.id, "created", previous_state=None)
    parties = await store.list_parties()
    return _party_to_response(party, parties)


@router.patch("/{party_id}", response_model=PartyResponse)
async def update_party(party_id: str, body: UpdatePartyRequest):
    from uuid import UUID
    party_uuid = UUID(party_id)
    party = await store.get_party(party_uuid)
    if party is None:
        raise HTTPException(status_code=404, detail="Party not found")

    previous_state = await store.get_party(party_uuid)
    updates: dict = {}
    ...
    updated = await store.update_party(party_uuid, updates)
    if updated is None:
        raise HTTPException(status_code=404, detail="Party not found")

    action = body.status or "updated"
    await store.add_action_log(party_uuid, action, previous_state=previous_state)
    parties = await store.list_parties()
    return _party_to_response(updated, parties)
...
async def delete_party(party_id: str):
    ...
    if party is None:
        raise HTTPException(status_code=404, detail="Party not found")
    await store.delete_party(party_uuid)
    return None


@router.post("/{party_id}/undo", response_model=PartyResponse)
async def undo_last_action(party_id: str):
    from uuid import UUID
    party_uuid = UUID(party_id)
    party = await store.get_party(party_uuid)
    if party is None:
        raise HTTPException(status_code=404, detail="Party not found")
    updated = await store.undo_last_action(party_uuid)
    parties = await store.list_parties()
    return _party_to_response(updated, parties)


@router.get("/token/{token}", response_model=PartyResponse)
async def get_party_by_token(token: str):
    party = await store.get_party_by_token(token)
    if party is None:
        raise HTTPException(status_code=404, detail="Party not found")
    parties = await store.list_parties()
    return _party_to_response(party, parties)
...
async def confirm_waiting(token: str):
    party = await store.get_party_by_token(token)
    if party is None:
        raise HTTPException(status_code=404, detail="Party not found")
    ...
    previous_state = await store.get_party(party.id)
    updated = await store.update_party(party.id, {"status": PartyStatus.notified, "notified_at": datetime.now(timezone.utc)})
    if updated is None:
        raise HTTPException(status_code=404, detail="Party not found")
    await store.add_action_log(party.id, "confirmed", previous_state=previous_state)
    parties = await store.list_parties()
    return _party_to_response(updated, parties)
...
async def cancel_by_token(token: str):
    ...
    previous_state = await store.get_party(party.id)
    updated = await store.update_party(party.id, {"status": PartyStatus.canceled, "canceled_at": datetime.now(timezone.utc)})
    if updated is None:
        raise HTTPException(status_code=404, detail="Party not found")
    await store.add_action_log(party.id, "canceled", previous_state=previous_state)
    parties = await store.list_parties()
    return _party_to_response(updated, parties)
