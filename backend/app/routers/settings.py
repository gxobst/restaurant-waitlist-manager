...
async def change_pin_endpoint(body: PinChangeRequest) -> None:
    ...
    await store.set_setting("manager_pin_hash", hash_pin(body.new_pin))


@router.get("/avg-turnover-time")
async def get_avg_turnover_time() -> int:
    value = await store.get_setting("avg_turnover_time")
    if value is None:
        return 30
    return int(value)
...
async def set_avg_turnover_time(body: SetMinutesRequest) -> None:
    ...
    await store.set_setting("avg_turnover_time", str(body.minutes))


@router.get("/waitlist-paused")
async def get_waitlist_paused() -> bool:
    value = await store.get_setting("waitlist_paused")
    if value is None:
        return False
...
async def set_waitlist_paused(body: SetPausedRequest) -> None:
    await store.set_setting("waitlist_paused", str(body.paused))
